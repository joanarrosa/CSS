...
export async function matchRulesAgainstDom(page, rules) {
  ...
  const result = await page.evaluate((rulesData) => {
    ...
    // Clean up the expando so we don't leave traces on the page.
    for (const eIdx of elementMeta.keys()) {
      // no direct handle retained; expando is harmless and page will be closed.
    }
