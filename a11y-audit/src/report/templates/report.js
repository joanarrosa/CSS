(function () {
  const data = window.__A11Y_REPORT_DATA__;
  ...
  function renderChipGroup(containerId, key, values, label) {
    ...
    el.innerHTML = values
      .map((v) => `<button class="chip ${state[key] === v ? "active" : ""}" data-key="${key}" data-value="${escapeHtml(v)}">${escapeHtml(label(v))}</button>`)
      .join("");
    ...
    return `
      ...
      f.occurrences
        ...
        <div class="finding-body ${hasShot ? "has-shot" : ""}">
          ...
          ${hasShot ? `<div class="finding-shot"><img src="${escapeHtml(f.screenshot)}" data-full="${escapeHtml(f.screenshot)}" alt="Evidence screenshot for ${escapeHtml(f.title)}" /></div>` : ""}
        </div>
      </div>`;
  }
